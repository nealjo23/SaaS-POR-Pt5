...
class BookSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $seedBooks = [
            ...
            ["title" => "Christmas Carol, A", "author" => "Dickens, Charles", "genre" => "fiction", "sub_genre" => "classic", "height" => 196, "publisher" => "Reprodukt",],
        ];
        ...
        foreach ($seedBooks as $book) {
            if (!isset($book['author']) || empty($book['author'])) {
                $author = Author::whereGivenName("UNKNOWN")->whereFamilyName("AUTHOR")->first();
            }
            else {
                $author = $book['author'];
                ...
                if (($comma === 0) || ($comma > 0)) {
                    ...
                    if (($space === 0) || ($space > 0)) {
                        ...
                        // There is no space or comma so must be one word
                        $authorGiven = $author;
                        $authorFamily = "";
                    }
                }
            ...
            if (!(is_null($book['publisher']))) {
                // Check if publisher exists, if not, add new publisher
                $publisher = Publisher::whereName($book['publisher'])->first();
            ...
            if (!(is_null($book['sub_genre']))) {
                // Check if sub_genre exists, if not, add new sub_genre
                $subGenre = Genre::where('name', $book['sub_genre'])->first();
                if (is_null($subGenre)) {
                    $newSubGenre = [
                        "name" => $book['sub_genre'],
                    ];
                    $subGenre = Genre::create($newSubGenre);
                }
            }
            else {
                $subGenre = null;
            ...
            $newBook = [
                'title' => $book['title'],
                'subtitle' => $book['subtitle'] ?? null, // Use null if no value present
